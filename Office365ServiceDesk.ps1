...
function Connect-365_Profile {
    ...
    Catch {
        Write-Warning "Fail to conenct to tenant:
    - There is no internet connction 
    - Login credential are correct
    - No more than 3 PSSessions can be connceted to a 365 tenant at once"
        Write-Verbose 'Type $Error to get a more details' -Verbose
    }
}
...
# Check for and/or install MSonline
if (-not (Get-InstalledModule -Name "MSonline")) {
    try {
...
$ShortcutFile = "$env:USERPROFILE\Desktop\Connect365.lnk"
